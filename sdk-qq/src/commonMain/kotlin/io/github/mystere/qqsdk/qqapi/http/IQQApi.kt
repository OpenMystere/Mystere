...
//    msgType: Int,
    ...
    var hasRawImageFile = false
    for (item in images) {
        if (item.startsWith("file:///") || item.startsWith("base64://")) {
            hasRawImageFile = true
            break
        }
    }
    return if (hasRawImageFile) {
        _messageIO_channel(channelId, MultiPartFormDataContent(formData {
//            append("msg_type", msgType)
        ...
        return _messageIO_channel(channelId, MessageRequestDto(
            content = content,
            embed = embed,
            ark = ark,
